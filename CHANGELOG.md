...
### Fixed
- The cleaning-mode menu now uses the theme's opaque card surface (`--card-background-color`, HA dialog convention) instead of a translucent tint: its options were unreadable over the map on themes with translucent cards.
- Action buttons harmonized with the glass design system: the secondary button (Dock) switches from a flat `--secondary-background-color` slab to a glass tint with hairline border, and the primary button gains a tinted hairline.

## [5.9.1] - 2026-07-02

### Fixed
- Theme-proof header and map surface: the header band and the map area no longer paint their own background (the map is clipped below the header, so nothing ever scrolls under it). On themes with translucent cards (`--ha-card-background` in rgba), the card now renders as a single uniform surface instead of showing an opaque band and a two-tone map zone. Both remain overridable via `--map-card-zoomer-background`.
...
### Changed
- Declared the minimum supported Home Assistant version (`2024.1.0`) in `hacs.json` so HACS can filter the card by compatibility.
- Hardened repository tooling: explicit read-only workflow permissions, per-job timeouts, and a daily security-audit pipeline (npm audit, Gitleaks, dependency review, lockfile freshness) alongside stricter GitHub Actions allow-listing. Test stack updated.
...
### Added
- Suite de tests Vitest portée de 23 à **651 tests** ; couverture **6 % → 44 %** (couche logique ≥ 90 %). Nouveaux fichiers pour validators, map_mode, coordinates, map_objects, utils, localize, composants Lit et robot-marker.
- Option `robot_overlay` exposée dans l'éditeur visuel (groupe « Map behavior ») + labels EN/FR + documentation README (dépendance « masquer le robot du PNG »).
- `ARCHITECTURE.md` (flux de données, structure, points d'extension) et déclaration de types `pointer-tracker.d.ts`.
- CI : job dédié `typecheck` + `test` dont dépend le build ; le workflow de release exécute aussi typecheck + tests.
- Constantes d'états vacuum centralisées dans `const.ts` (`ACTIVE_VACUUM_STATES`, `ACTIVELY_CLEANING_STATES`, `RESTING_VACUUM_STATES`).

### Fixed
- **Fuite mémoire** : la directive action-handler accumulait 7 listeners `document` à chaque (re)connexion sans les retirer → `AbortController` + `disconnectedCallback`.
- `predefined-multi-rectangle` / `predefined-point` : `getFromEntities` plantait (TypeError) ou produisait des coordonnées `NaN` sur entité absente ou état `unavailable`/non-JSON → filtrage robuste.
- `manual-rectangle` : tri lexicographique des bornes remplacé par `Math.min`/`Math.max` (bornes fausses sur coordonnées négatives/multi-chiffres).
- `config-validators` : précédence `?? >` corrigée ; gardes sur `zones`/`id` ; `validateCalibrationPoint` ne plante plus sur un point sans `map`/`vacuum`.
- Accès non gardé à l'état d'une entité absente sécurisé dans le composant principal.
- `_handleLovelaceDomEvent` : gardes sur `data` + bornage des répétitions (plus de `RangeError`).
- i18n : `blank_before_percent` utilisait `cz` au lieu de `cs` (tchèque) ; `compute_domain` tronquait un id sans point.
- robot-animation : markup en retard d'un rendu (dérivation d'état déplacée dans `willUpdate()`).
- `getMousePosition` : coordonnées `undefined`/`NaN` sur événement non géré → init à 0.
- Sécurité : `handleAction` (action `url`) ouvre avec `noopener` et refuse le schéma `javascript:`.
- Appels de service : `evaluateJinjaTemplate` a un timeout (plus de promesse en suspens) ; `replaceInStr` ne plante plus sur JSON invalide ; `ServiceCallSchema` garde un service malformé.
- `robot_overlay` : réinitialisation des buffers d'image au changement de caméra (évite l'affichage transitoire de la carte d'un autre robot).

### Changed
- Refactor structurel : `static styles` (~600 l.) extrait dans `card-styles.ts` ; `utils.ts` (302 l.) éclaté en modules à responsabilité unique (`utils/`) ; cycle d'import carte ↔ utils cassé (`import type`) ; `Context` passé de 21 paramètres positionnels à un objet nommé `ContextOptions`.
- Typage strict : `noImplicitAny: true` activé (18 implicit-any corrigés).
- Accessibilité : `cleaning-mode-chip` (role/tabindex/clavier/aria-label), `tab-selector` (`role="tablist"/"tab"` + `aria-selected`).

### Removed
- **~2200 lignes de code mort** : famille de composants `xvmc-*` (héritée du fork, non utilisée), générateurs d'icônes/tuiles, `ToastRenderer`, classes `Obstacle`/`Furniture`, polyfills obsolètes, 32 exports inutilisés (helpers HA portés non câblés), méthodes orphelines de `platform-generator`.
- Pipeline Babel du build (redondant : TypeScript compile déjà en es2021 en amont) → 4 devDependencies + `.babelrc.json`.

### Connu / restant
- Couverture globale plafonnée par la glu DOM/canvas (composant principal, `pinch-zoom`, `editor`, `action-handler`) non testable en happy-dom : un harnais navigateur réel (Playwright / @web/test-runner) serait nécessaire.
- Convention de l'angle `vacuum_position.a` (robot-overlay) à valider sur matériel réel.
- Bug latent : 3 points de calibration colinéaires passent silencieusement (matrice dégénérée non détectée). Reconnexion d'un `dreame-robot-animation` à animation lottie déjà chargée : pas de rechargement (masqué car HA recrée l'élément).
