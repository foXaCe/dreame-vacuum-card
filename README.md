...
### Robot overlay (anti-flash)

By default the robot position is "baked" into the camera PNG by the integration, which makes it flicker every time the image reloads (~5 FPS). Enable `robot_overlay: true` to instead draw the robot as a separate HTML element, positioned from the camera's `vacuum_position` attribute, with CSS transitions that smoothly interpolate position changes. The card also double-buffers the map image (preload + decode off-screen before swapping) to remove the white flash on reload.

> **Important:** when `robot_overlay` is enabled, **hide the robot icon from the integration's rendered PNG** (option _Robot Icon_ in the integration's _Hidden map objects_). Otherwise two robots are displayed — the baked-in one and the overlay.

The heading is derived from `vacuum_position` (`x`, `y`, `a`) through the map calibration, so it stays correct regardless of map rotation or perspective.
