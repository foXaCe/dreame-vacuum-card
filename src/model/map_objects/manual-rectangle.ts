...
export class ManualRectangle extends MapObject {
    ...
    private _toVacuumFromDimensions(
        ...
        const minX = Math.min(vacuumStart[0], vacuumEnd[0]);
        const maxX = Math.max(vacuumStart[0], vacuumEnd[0]);
        const minY = Math.min(vacuumStart[1], vacuumEnd[1]);
        const maxY = Math.max(vacuumStart[1], vacuumEnd[1]);
