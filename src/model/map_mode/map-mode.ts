    ...
    public async getServiceCall(
        ...
        if (this.serviceCallSchema.evaluateDataAsTemplate) {
            try {
                ...
                // Échec d'évaluation ou de parsing du template : on conserve l'appel de service
                // non évalué (dégradation gracieuse) plutôt que de bloquer l'action.
                if (MapMode.debug) console.error("Échec de l'évaluation du template du service", e);
            }
        }
        return serviceCall;
    }
