import { MapModeConfig } from "./types/types";
import { SelectionType } from "./model/map_mode/selection-type";
import { RepeatsType } from "./model/map_mode/repeats-type";

export const CARD_VERSION = "v@VACUUM_MAP_CARD_VERSION_PLACEHOLDER@";
export const CARD_CUSTOM_ELEMENT_NAME = "dreame-vacuum-card";
export const EDITOR_CUSTOM_ELEMENT_NAME = "dreame-vacuum-card-editor";
export const ACTION_HANDLER_CUSTOM_ELEMENT_NAME = "action-handler-dreame-vacuum-card";
/** Délai (ms) après lequel l'absence de mise à jour de la caméra est traitée comme une
 *  déconnexion (l'image « déconnecté » est alors affichée). 360000 ms = 6 minutes. */
export const DISCONNECTION_TIME = 360000;
export const DISCONNECTED_IMAGE =
    "data:image/png;base64,";
export const EVENT_SELECTION_CHANGED = "map-card-selection-changed";
export const EVENT_AUTOGENERATED_CONFIG_GET = "map-card-autogenerated-config-get";
export const EVENT_AUTOGENERATED_CONFIG = "map-card-autogenerated-config";
export const EVENT_ROOM_CONFIG_GET = "map-card-room-config-get";
export const EVENT_ROOM_CONFIG = "map-card-room-config";
export const EVENT_SERVICE_CALL_GET = "map-card-service-call-get";
export const EVENT_SERVICE_CALL = "map-card-service-call";
export const EVENT_LOVELACE_DOM = "ll-custom";
export const EVENT_LOVELACE_DOM_DETAIL = "dreame_vacuum_card";

/** États de l'entité vacuum (intégration dreame_vacuum). */
export const VacuumState = {
    CLEANING: "cleaning",
    SEGMENT_CLEANING: "segment_cleaning",
    ZONED_CLEANING: "zoned_cleaning",
    SPOT_CLEANING: "spot_cleaning",
    RETURNING: "returning",
    PAUSED: "paused",
    DOCKED: "docked",
    IDLE: "idle",
    CHARGING: "charging",
    CHARGING_COMPLETED: "charging_completed",
} as const;

/** États où le robot se déplace / nettoie activement (icône et overlay « actifs »). */
export const ACTIVE_VACUUM_STATES: readonly string[] = [
    VacuumState.CLEANING,
    VacuumState.SEGMENT_CLEANING,
    VacuumState.ZONED_CLEANING,
    VacuumState.SPOT_CLEANING,
    VacuumState.RETURNING,
];

/** États indiquant un nettoyage par pièces en cours (lecture de l'attribut `active_segments`). */
export const ACTIVELY_CLEANING_STATES: readonly string[] = [
    VacuumState.CLEANING,
    VacuumState.SEGMENT_CLEANING,
    VacuumState.ZONED_CLEANING,
    VacuumState.PAUSED,
];

/** États de repos : robot à la base ou inactif. */
export const RESTING_VACUUM_STATES: readonly string[] = [
    VacuumState.DOCKED,
    VacuumState.IDLE,
    VacuumState.CHARGING,
    VacuumState.CHARGING_COMPLETED,
];

export const EMPTY_MAP_MODE: MapModeConfig = {
    run_immediately: true,
    selection_type: SelectionType[SelectionType.ROOM],
    repeats_type: RepeatsType[RepeatsType.NONE],
};
