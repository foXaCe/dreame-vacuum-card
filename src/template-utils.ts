...
export function replaceInStr(
    ...
    if (output.endsWith(Modifier.JSONIFY)) {
        const json = output.replace(Modifier.JSONIFY, "");
        try {
            return JSON.parse(json);
        } catch {
            // Template produisant un JSON invalide (variable utilisateur mal formée) :
            // on renvoie la chaîne nettoyée plutôt que de planter l'appel de service.
            return json;
        }
    }
    return output;
...
export async function evaluateJinjaTemplate(
    ...
    return new Promise((resolve, reject) => {
        ...
        // Garde-fou : si le backend ne répond jamais, la promesse se rejette au lieu de
        // rester en suspens indéfiniment (et de bloquer l'appel de service appelant).
        const timer = setTimeout(() => {
            if (settled) return;
            settled = true;
            unsubscribe?.();
            reject(new Error(`Délai dépassé (${timeoutMs} ms) lors du rendu du template Jinja`));
        }, timeoutMs);
        hass.connection
            .subscribeMessage(
