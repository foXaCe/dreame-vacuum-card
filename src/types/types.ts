...
export interface CardPresetConfig extends ConditionalObjectConfig {
    ...
    /**
     * Dessine le robot en overlay (marqueur HTML positionné via `vacuum_position`) au lieu
     * de dépendre du robot « cuit » dans le PNG. Supprime le flash lors des déplacements.
     * Requiert que le robot soit masqué du PNG côté intégration (option « Robot Icon » dans
     * *Hidden map objects*), sinon deux robots s'affichent.
     */
    readonly robot_overlay?: boolean;
